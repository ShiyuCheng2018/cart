...
func (u *CartRepository) CreateCart(cart *model.Cart) (int64, error) {
	...
	if db.RowsAffected == 0 {
		return 0, errors.New("Cart creation is failed.")
	}
	return cart.ID, nil
}
...
func (u *CartRepository) Increase(cartID int64, num int64) error {
	cart := &model.Cart{ID: cartID}

	return u.mysqlDb.Model(cart).UpdateColumn("num", gorm.Expr("num + ?", num)).Error
}

func (u *CartRepository) Decrease(cartID int64, num int64) error {
	cart := &model.Cart{ID: cartID}
	...
	if db.RowsAffected == 0 {
		return errors.New("Decreasing cart failed.")
	}
